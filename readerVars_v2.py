...
# Date: June 2024
# File Purpose: .py file to read the files necessary for temperature and vapor
#               density retrieval from spectrometer (SINGLE ENTRY PER TIMESLOT)
# %% ------------------------------------------------------------------------
import os
import numpy as np
import pandas as pd
...
VarX = np.concatenate((f['Tb'][0][0:7],
                       f['Tb'][0][8:],
                       f['Rain'][:]))
VarX = np.reshape(VarX, (1, -1)) # Reshape to one row
...
with nc.MFDataset(fns,'r') as fs:
  ...
  RH = fs['RH'][:].filled(fill_value=np.nan)
  # Put the 0 m data (T, P, RH) as the training data
  Ts = fs['T'][:].filled(fill_value=np.nan)
  Ps = fs['P'][:].filled(fill_value=np.nan)
  RHs = fs['RH'][:].filled(fill_value=np.nan)
  print(T.shape)
  print(Qv.shape)
...
for file in fileRS41nST:
  ...
  RH_file = f['RH'][:].filled(fill_value=np.nan)
  # Put the 0 m data (T, P, RH) as the training data
  Ts_file = f['T'][:].filled(fill_value=np.nan)
  Ps_file = f['P'][:].filled(fill_value=np.nan)
  RHs_file = f['RH'][:].filled(fill_value=np.nan)
  T = np.concatenate((T, T_file), axis=0)
  Qv = np.concatenate((Qv, Qv_file), axis=0)
  Ts = np.concatenate((Ts, Ts_file), axis=0)
  Ps = np.concatenate((Ps, Ps_file), axis=0)
  RH = np.concatenate((RH, RH_file), axis=0)
  RHs = np.concatenate((RHs, RHs_file), axis=0)
  
# %% ------------------------------------------------------------------------
# Subset the 0 m data (T, P, RH) as the training data
Ts = np.reshape(Ts[:,0], (-1, 1))
Ps = np.reshape(Ps[:,0], (-1, 1))
RHs = np.reshape(RHs[:,0], (-1, 1))
# %% ------------------------------------------------------------------------
VarX = np.concatenate((VarX, Ts, Ps, RHs), axis=1)
# %% ------------------------------------------------------------------------
# * Subset the x values with Rain == 0 (sunny) to be kept for training'
# column index 34 is 'Rain'
# rainy data will not be used
rainIdx = 34
VarX_sunny = VarX[VarX[:, rainIdx] == 0]
VarX_sunny = np.delete(VarX_sunny, rainIdx, axis=1) # remove the 'Rain' column after subsetting
# %% ------------------------------------------------------------------------
# * Pick only rows that corresponds to sunny lvl1 data
T_sunny = T[VarX[:, rainIdx] == 0]
Qv_sunny = Qv[VarX[:, rainIdx] == 0]
RH_sunny = RH[VarX[:, rainIdx] == 0]
print(T_sunny.shape)
print(Qv_sunny.shape)
print(RH_sunny.shape)
...
print((np.isnan(T_sunny).any(axis=1)==np.isnan(RH_sunny).any(axis=1)))
...
RH_train = RH_sunny[~np.isnan(T_sunny).any(axis=1)]
...
RH_test = RH_sunny[np.isnan(T_sunny).any(axis=1)]
print(T_train.shape)
...
RH_train_augmented = np.concatenate(
  (RH_train, RH_train_copy, RH_train_copy), axis=0)
...
for fn in fns[1:]:
  ...
  RH_lv2 = np.concatenate((RH_lv2, f['RH'][:].filled(fill_value=np.nan)), axis=0)
print(T_lv2.shape)
print(Qv_lv2.shape)
# %% ------------------------------------------------------------------------
# subset the sunny data, then split into training and test
T_lv2_sunny = T_lv2[VarX[:, rainIdx] == 0]
Qv_lv2_sunny = Qv_lv2[VarX[:, rainIdx] == 0]
RH_lv2_sunny = RH_lv2[VarX[:, rainIdx] == 0]
T_lv2_train = T_lv2_sunny[~np.isnan(T_sunny).any(axis=1)]
Qv_lv2_train = Qv_lv2_sunny[~np.isnan(Qv_sunny).any(axis=1)]
RH_lv2_train = RH_lv2_sunny[~np.isnan(RH_sunny).any(axis=1)]
# %% ------------------------------------------------------------------------
# triple the data
T_lv2_train_copy = T_lv2_train.copy()
Qv_lv2_train_copy = Qv_lv2_train.copy()
RH_lv2_train_copy = RH_lv2_train.copy()
T_lv2_train_augmented = np.concatenate(
  (T_lv2_train, T_lv2_train_copy, T_lv2_train_copy), axis=0)
Qv_lv2_train_augmented = np.concatenate(
  (Qv_lv2_train, Qv_lv2_train_copy, Qv_lv2_train_copy), axis=0)
RH_lv2_train_augmented = np.concatenate(
  (RH_lv2_train, RH_lv2_train_copy, RH_lv2_train_copy), axis=0)
T_lv2_test = T_lv2_sunny[np.isnan(T_sunny).any(axis=1)]
Qv_lv2_test = Qv_lv2_sunny[np.isnan(Qv_sunny).any(axis=1)]
RH_lv2_test = RH_lv2_sunny[np.isnan(RH_sunny).any(axis=1)]
...
# %%
np.savetxt("train_y_RH.csv", RH_train_augmented, delimiter=",")
np.savetxt("eval_y_RH.csv", RH_test, delimiter=",")
np.savetxt("train_y_RH_lv2.csv", RH_lv2_train_augmented, delimiter=",")
np.savetxt("eval_y_RH_lv2.csv", RH_lv2_test, delimiter=",")
np.savetxt("train_y_RH_original.csv", RH_train, delimiter=",")
np.savetxt("train_y_RH_lv2_original.csv", RH_lv2_train, delimiter=",")
# %%
